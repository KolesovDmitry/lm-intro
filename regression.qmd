...
* Чем отличаются подходы к моделированию в машинном обучении и в статистике?

* Регрессия используется:
    - восстанавливаем функциональную зависимость
    - A/B тесты
...
## 1. Пример 1: парная линейная регрессия
Парная регрессия - только две переменные (x и Y).
...
* Подозрение: новый менеджер по логистике "дружит" с неким перевозчиком и проводит счета по завышенным ставкам.
...
**Особенности:** Нормальное распределение. МНК приводит к регрессии с нормально распределенными ошибками
$\varepsilon_i \sim N(0, \sigma)$ (и наоборот).
...
const (-3.1469): свободный член

x (0.2228): коэффициент x
...
Prob (F-statistic) (1.06e-06): статистическая значимость модели
...
**Предикторы и Нулевая гипотеза:**

По умолчанию считается, что признак на самом деле ничего не значит (его коэффициент равен нулю). p-value показывает вероятность того, что мы ошиблись бы, если бы отбросили это предположение.

Обычно:

* Если p-value < 0.05, признак важен.
* Если p-value > 0.05, признак, скорее всего, бесполезен (шум), и его можно удалить из модели.

**Модель в целом:**
*F-статистика* отвечает на вопрос: есть ли вообще смысл в этой модели, или она не лучше, чем просто среднее значение?

* Принцип: она сравнивает модель (со всеми признаками) с примитивной моделью, которая просто предсказывает среднюю цифру.

* Чем больше F-статистика, тем сильнее ваша модель лучше, чем "тупое" угадывание среднего. Для F-статистики тоже считается p-value. Если p-value большой - модель плохая.
...
PS. Кстати, помните, что я в начале упоминал A/B тесты?
...
PS. Еще раз напомню про A/B тесты.
...
* Грубое правило:
    ...
    * Разница от 4 до 7: есть основания предпочесть модель с меньшим значением.
...
Менеджер собрал данные по стоимости доставки:

* $dist$ - длина пути, $wgt$ -- вес груза, $vol$ -- объем груза;
* $cost$ - цена подрядчика.
...
data.cost = data.cost.apply(lambda x: round(x))
data.dist = data.cost.apply(lambda x: round(x))
data.head(5)
```

Вопрос: какова среднерыночная цена доставки в зависимости от веса, объема и расстояния?
...
## Промежуточный вывод по примеру 2

Добавление предиктора (объем груза) испортило модель. Почему?

```{python}
#| echo: true
 
data.plot.scatter(x='wgt', y='vol')

```

Сильная корреляция между предикторами.

## Мультиколинеарность

Мультиколинеарность - сильная линейная связь между предикторами.

Предикторы "перетягивают" одеяло на себя, модель "не понимает", кому из предикторов какой коэффициент назначить.

Признаки мультиколинераности:

* на графиках
* статистичеки значимая модель, но не значимые коэффициенты.

**Еще пример мультиколинеарности:** категориальные признаки.

* Y - выручка за день (по будням больше);
* X - день недели;

Кодируем день недели (`pandas.get_dummies(data, drop_first=False, ...)`):

:::: {.columns}

::: {.column width="50%"}
* неправильно: 7 признаков
    * пнд: 1, 0, ..., 0, 0
    * втр: 0, 1, ..., 0, 0
    * ...
    * сбт: 0, 0, ..., 1, 0
...
## Нелинейные предикторы

Результат работы линейной регрессии - прямая? (плоскоть, гиперплоскость?). Можно получить на выходе нелинейное значение зависимой величины Y?

Модель линейна, если коэффициенты в модели линейны:

    * $Y = b_0 +b_1 x$
    * $Y = b_0 +b_1 x + b_2 x^2$
    * $Y = b_0 +b_1 x + b_2 x^2 + b_3 sin(x)$
    * ....

Пример: Prophet - линейная модель

![](img/regression/prophet.jpg){fig-align="center" width=40%}

$$
Y = trend + a_1\sin(t)+b_1\cos(t) + a_2\sin(2t)+b_2\cos(2t) + \dots + a_k\sin(kt)+b_k\cos(kt) 
$$


## Общие выводы

Линейная регрессия - очень мощный инструмент. Но есть подводные камни.
